...
import { Truck, MapPin, Clock, CreditCard, User, HeadphonesIcon, Wallet, TrendingUp } from 'lucide-react-native';
  ...
  useEffect(() => {
    const fetchRecent = async () => {
      try {
        ...
        // Fetch all completed to calculate total spent (naive approach since no dedicated endpoint exists yet)
        const allRes = await api.get('/bookings?status=DELIVERED');
        if (allRes.data?.data) {
          const total = allRes.data.data.reduce((sum: number, b: any) => sum + (b.finalFare || b.fareEstimate || 0), 0);
          setTotalSpent(total);
        }
      } catch (e) {
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            <View style={[styles.statIcon, { backgroundColor: 'rgba(55, 138, 221, 0.1)' }]}>
              <TrendingUp size={18} color={theme.colors.brand.primary} />
            </View>
            <View>
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: theme.colors.background.card, borderRadius: 16, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, borderWidth: 1, borderColor: theme.colors.border.subtle, ...theme.shadows.sm },
  statIcon: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center' },
  statLabel: { color: theme.colors.text.muted, fontSize: 11, fontWeight: '600', marginBottom: 2 },
